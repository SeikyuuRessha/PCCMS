...
class BoardingCareLogServiceImplTest {
    ...
    private BoardingCareLogServiceImpl boardingCareLogService;

    @BeforeEach
    void setUp() {
        boardingCareLogService = new BoardingCareLogServiceImpl(sql, securityHelper, fileMediaService);
    }
    ...
    void uploadMediaUsesFileMediaServiceInsteadOfWritingFakeFileAssetKey() {
        ...
        given(sql.jdbc()).willReturn(jdbcTemplate);
        ...
        verify(sql, never()).one(contains("INSERT INTO file_assets"), any());
