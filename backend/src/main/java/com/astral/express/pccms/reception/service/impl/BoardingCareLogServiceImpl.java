...
public class BoardingCareLogServiceImpl implements BoardingCareLogService {
    ...
    public CareLogResponse saveCareLog(CareLogRequest request) {
        LocalDate logDate = sql.date(request.logDate(), LocalDate.now());
        ...
        if (sessionId == null) {
            Map<String, Object> session = sql.optional("""
                    SELECT bs.id
        ...
        if (petId == null) {
            Map<String, Object> sessionPet = sql.one("""
                    SELECT b.pet_id
                    FROM boarding_sessions bs
                    JOIN boarding_bookings b ON b.id = bs.booking_id
                    WHERE bs.id = ?
        ...
        Map<String, Object> row = sql.one("""
                ...
                ON CONFLICT(session_id, log_date, period_code) DO UPDATE SET
                    ...
                    updated_at = now()
                RETURNING id
                """, sessionId, petId, securityHelper.getCurrentUserId(), logDate, request.periodCode(), request.feedingStatus(),
