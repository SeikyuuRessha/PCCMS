...
describe('medicineApi', () => {
  ...
  it('getMedicines should fetch list of medicines', async () => {
    ...
    expect(axiosClient.get).toHaveBeenCalledWith('/api/v1/medicines', { params: { page: 1, size: 10 } });
  ...
  it('getMedicineById should fetch medicine details', async () => {
    ...
    expect(axiosClient.get).toHaveBeenCalledWith('/api/v1/medicines/med-123');
  ...
  it('createMedicine should post new medicine', async () => {
    ...
    expect(axiosClient.post).toHaveBeenCalledWith('/api/v1/medicines', request);
    expect(result).toEqual(mockMedicine);
  });

  it('updateMedicine should put updated medicine', async () => {
    const request: UpdateMedicineRequest = { name: 'Amox 500mg' };
    ...
    expect(axiosClient.put).toHaveBeenCalledWith('/api/v1/medicines/med-123', request);
    expect(result.name).toEqual('Amox 500mg');
  });

  it('addStock should patch medicine stock', async () => {
    const request: AddStockRequest = { quantity: 50 };
    ...
    expect(axiosClient.patch).toHaveBeenCalledWith('/api/v1/medicines/med-123/stock', request);
  ...
  it('deleteMedicine should delete medicine', async () => {
    ...
    expect(axiosClient.delete).toHaveBeenCalledWith('/api/v1/medicines/med-123');
  });
});
