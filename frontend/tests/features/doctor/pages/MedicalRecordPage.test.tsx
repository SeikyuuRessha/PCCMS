...
describe('MedicalRecordPage', () => {
  ...
  it('calls finalize API when Chốt bệnh án is clicked', async () => {
    vi.mocked(medicalRecordApi.getMedicalRecordById).mockResolvedValue(mockRecord);
    vi.mocked(medicalRecordApi.finalizeMedicalRecord).mockResolvedValue({ ...mockRecord, recordStatus: 'FINALIZED' } as any);
    ...
    await waitFor(() => {
      expect(medicalRecordApi.finalizeMedicalRecord).toHaveBeenCalledWith('record-123', expect.objectContaining({
