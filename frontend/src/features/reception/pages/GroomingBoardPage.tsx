...
export function GroomingBoardPage() {
    ...
    const completeMutation = useMutation({
        mutationFn: (ticketId: string) =>
            groomingApi.completeTicket(ticketId, "Hoàn thành dịch vụ làm đẹp"),
        onSuccess: (ticket) => {
            toast.success(
                ticket.invoice
                    ? `Đã tạo hóa đơn ${ticket.invoice.invoiceCode}`
    ...
    const cancelMutation = useMutation({
        mutationFn: (ticketId: string) =>
            groomingApi.cancelTicket(ticketId, "Nhân viên hủy lịch theo điều phối"),
        onSuccess: () => {
    ...
    if (ticketsQuery.isError || stationsQuery.isError) {
        return (
            <EmptyState title="Không thể tải bảng làm đẹp" description="Vui lòng thử lại sau." />
        );
    }
    ...
    return (
        <div className="space-y-6">
            ...
            {tickets.length === 0 ? (
                <EmptyState
                    title="Chưa có lịch làm đẹp"
                    description="Các yêu cầu mới của chủ nuôi sẽ hiển thị tại đây."
                />
            ) : (
                <div className="grid gap-4 xl:grid-cols-4">
                    {columns.map((column) => {
                        const columnTickets = tickets.filter((ticket) =>
                            column.statuses.includes(ticket.statusCode)
                        );
                        return (
                            <Card
                                key={column.title}
                                title={column.title}
                                right={<Tag tone={column.tone}>{columnTickets.length} phiếu</Tag>}
                            >
                                        ...
                                        columnTickets.map((ticket) => (
                                            <TicketCard
                                                ...
                                                onConfirm={() => {
                                                    ...
                                                    if (!stationId) {
                                                        toast.error(
                                                            "Chọn khu làm đẹp trước khi xác nhận"
                                                        );
                                                        return;
                                                    }
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            ...
            {ticket.invoice && (
                <div className="mt-3 rounded-xl bg-white p-3 text-sm text-emerald-800">
                    Hóa đơn {ticket.invoice.invoiceCode}:{" "}
                    {formatCurrency(ticket.invoice.totalAmountVnd)} chờ thanh toán
                </div>
            )}
        </div>
    );
