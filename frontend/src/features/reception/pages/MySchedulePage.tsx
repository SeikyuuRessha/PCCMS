...
type PersonalScheduleStatus = "Đã phân công" | "Đã hủy" | "Đã hoàn thành";
type PersonalScheduleViewMode = "Ngày" | "Tuần" | "Tháng";

type ShiftChangeRequestStatus = "Đang chờ" | "Đã đồng ý" | "Từ chối" | "Đã hủy";
...
export function MySchedulePage() {
    ...
    const loadFromApi = async (fromDate = defaultFilters.fromDate, toDate = defaultFilters.toDate) => {
        const [scheduleData, requestData] = await Promise.all([
            getMyWorkSchedules(fromDate, toDate),
            getMyShiftChangeRequests(),
        ]);
        setItems(scheduleData as PersonalSchedule[]);
        setRequests(requestData as ShiftChangeRequest[]);
    };

    useEffect(() => {
        void loadFromApi().catch(() => undefined);
    }, []);
    ...
    const handleSearch = async () => {
        ...
        setLoading(true);
        try {
            await loadFromApi(filters.fromDate, filters.toDate);
        } catch {
    ...
    const handleReset = () => {
        ...
        void loadFromApi().catch(() => {
            ...
            setRequests(baseShiftChangeRequests);
        });
    };
    ...
    const submitShiftChange = async (payload: { reason: string; receiverName: string }) => {
        ...
        const isPersistedRequest = isBackendId(selectedSchedule.id);
        if (isPersistedRequest) {
            request = {
                ...(await createMyShiftChangeRequest(selectedSchedule.id, payload.reason)),
        ...
        setDialogError("");
        setFeedback(isPersistedRequest ? "Gửi yêu cầu đổi ca thành công" : "Thao tác chỉ cập nhật tạm thời, chưa lưu lên hệ thống.");
    };
