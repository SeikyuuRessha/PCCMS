...
export function PrescriptionTable({ disabled = false }: PrescriptionTableProps) {
  const { control, register, setValue, formState: { errors } } = useFormContext<PrescriptionFormValues>();
          ...
          rows={fields.map((field, index) => {
            ...
            return [
              ...
              <div key={`calc-${field.id}`} className="flex items-center gap-2">
                <Input type="number" className="w-16" placeholder="Liều" disabled={disabled} {...register(`items.${index}.dosage`, { valueAsNumber: true })} />
                <span>x</span>
                <Input type="number" className="w-16" placeholder="Lần" disabled={disabled} {...register(`items.${index}.frequency`, { valueAsNumber: true })} />
                <span>x</span>
                <Input type="number" className="w-16" placeholder="Ngày" disabled={disabled} {...register(`items.${index}.durationDays`, { valueAsNumber: true })} />
              </div>,
