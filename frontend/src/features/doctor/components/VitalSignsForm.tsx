...
export const vitalSignsSchema = z.object({
  temperatureC: z.coerce.number().min(30, 'Nhiệt độ quá thấp').max(45, 'Nhiệt độ quá cao').optional().or(z.literal('')),
  heartRateBpm: z.coerce.number().min(0, 'Nhịp tim không hợp lệ').optional().or(z.literal('')),
  respiratoryRateBpm: z.coerce.number().min(0, 'Nhịp thở không hợp lệ').optional().or(z.literal('')),
  weightKg: z.coerce.number().min(0, 'Cân nặng phải >= 0').optional().or(z.literal('')),
  ...
  capillaryRefillSeconds: z.coerce.number().min(0, 'Thời gian CRT không hợp lệ').optional().or(z.literal('')),
      ...
      <form>
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          ...
          <Input 
            ...
            error={errors.spo2Percent?.message}
          />
          <Input 
            label="Màu niêm mạc" 
            disabled={disabled}
            {...register('mucousMembraneColor')}
            error={errors.mucousMembraneColor?.message}
          />
          <Input 
            label="CRT (giây)" 
            type="number"
            ...
            error={errors.capillaryRefillSeconds?.message}
          />
        </div>
        <div className="mt-4 grid gap-4">
          ...
          <Textarea
            ...
            error={errors.finalDiagnosis?.message}
          />
        </div>
        {!disabled && (
          <div className="mt-6 flex gap-2">
            ...
            <Button 
              variant="secondary" 
              type="button"
              onClick={handleSubmit((data) => onFinalize?.(data))}
              disabled={isSaving || isFinalizing}
            >
