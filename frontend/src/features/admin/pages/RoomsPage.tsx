...
export function RoomsPage() {
    ...
    const roomTypeOptions = useMemo(
        () =>
            rooms
                .filter((room) => room.roomTypeId && room.type)
                .map((room) => ({ id: room.roomTypeId as string, name: room.type }))
                .filter((roomType, index, all) => all.findIndex((item) => item.id === roomType.id) === index),
        [rooms]
    );
    ...
    const runSearch = async () => {
        const hasCriteria =
            ...
            filters.roomTypeId.trim() ||
            filters.status.trim() ||
            filters.floor.trim() ||
            filters.capacity.trim();
        if (!hasCriteria) {
    ...
    const confirmDeactivate = async () => {
        ...
        try {
            ...
            setRooms((prev) => prev.filter((item) => item.id !== deleteTarget.id));
            setDeleteTarget(null);
            setFeedback("Ngừng hoạt động phòng lưu trú thành công");
        } catch (error) {
            setDeleteError(error instanceof Error ? error.message : "Không thể ngừng hoạt động phòng");
        } finally {
            setDeleteLoading(false);
        }
    };
    ...
    return (
        <div className="space-y-6">
            ...
            <RoomFilters value={filters} roomTypeOptions={roomTypeOptions} onChange={setFilters} onSearch={runSearch} onReset={resetFilters} loading={loading} error={searchError} />
