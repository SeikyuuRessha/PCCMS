...
const fallbackMedicineCategoryOptions = medicineGroups.map((name) => ({ id: `local:${name}`, name }));
...
export function CatalogPage() {
    ...
    const medicineCategoryOptions = useMemo(
        () => {
            const backendOptions = medicines
                .filter((medicine) => medicine.categoryId && medicine.group)
                .map((medicine) => ({ id: medicine.categoryId as string, name: medicine.group }))
                .filter((category, index, all) => all.findIndex((item) => item.id === category.id) === index);

            return [...backendOptions, ...fallbackMedicineCategoryOptions].filter(
                (category, index, all) =>
                    all.findIndex((item) => item.id === category.id || item.name === category.name) === index
            );
        },
        [medicines]
    );
        ...
        <div className="space-y-6">
            ...
            {isMedicineTab ? (
                <div className="space-y-6">
                    ...
                    <MedicineFilters value={medicineFilters} categoryOptions={medicineCategoryOptions} onChange={setMedicineFilters} onSearch={runMedicineSearch} onReset={resetMedicineFilters} loading={medicineLoading} error={medicineSearchError} />
