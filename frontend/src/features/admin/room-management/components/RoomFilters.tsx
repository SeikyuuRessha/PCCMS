...
import { roomStatusLabels, roomStatuses } from "../mockRooms";
...
export function RoomFilters({ value, roomTypeOptions, onChange, onSearch, onReset, loading, error }: RoomFiltersProps) {
    return (
        <Card title="Bộ lọc tìm kiếm" subtitle="Tìm theo mã phòng, tên phòng, loại phòng, trạng thái, tầng và sức chứa">
            <div className="grid gap-4 lg:grid-cols-5">
                ...
                <div className="flex flex-col gap-1.5">
                    ...
                    <select
                        ...
                        onChange={(e) => {
                            const roomType = roomTypeOptions.find((item) => item.id === e.target.value);
                            onChange({
                                ...value,
                                roomTypeId: roomType?.id ?? "",
                                type: (roomType?.name ?? "") as RoomType | "",
                            });
                        }}
                    >
